...
use alloy_core::primitives::Sign;
...
impl EthDecodedLogsToArrow {
    ...
    pub fn new_from_signature(signature: &str) -> Result<Self, alloy_core::dyn_abi::Error> {
...
impl Transcoder for EthDecodedLogsToArrow {
    ...
    fn append(&mut self, logs: &[Log]) -> Result<(), AppendError> {
        for log in logs {
            let decoded = self.event_decoder.decode_log_data(log.data())?;
